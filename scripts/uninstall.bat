...
:remove_path_entries
powershell -NoProfile -ExecutionPolicy Bypass -Command "$key='HKCU:\Software\my-cp-setup'; $esc=[char]27; $removed='['+$esc+'[38;5;114mREMOVED'+$esc+'[0m]'; $clear='['+$esc+'[38;5;114mCLEAR'+$esc+'[0m]'; $owned=@((Get-ItemProperty -Path $key -Name 'Path.Entries' -ErrorAction SilentlyContinue).'Path.Entries' | Where-Object { $_ } | ForEach-Object { $_.TrimEnd('\') }); if (-not $owned.Count) { Write-Host ($clear+' User PATH entries managed by setup'); exit 0 }; $path=[Environment]::GetEnvironmentVariable('Path','User'); if ($null -eq $path) { $path='' }; $kept=@($path -split ';' | Where-Object { $_ -and -not ($owned -icontains $_.Trim().TrimEnd('\')) }); [Environment]::SetEnvironmentVariable('Path',($kept -join ';'),'User'); Remove-ItemProperty -Path $key -Name 'Path.Entries' -ErrorAction SilentlyContinue; Write-Host ($removed+' User PATH entries managed by setup')"
exit /b %ERRORLEVEL%
